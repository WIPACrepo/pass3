...
def check_input_file(file: Path):
    '''
    Check that the input file is a .npz file containing the required arrays 'atwd_mean' and 'fadc_mean'. This is just a helper function to keep the code cleaner.

    - file: location of the run data file to use, should be a .npz file containing 'atwd_mean' and 'fadc_mean' arrays for the data, which will be histogrammed in the same way as the templates. The output will be two flattened arrays of the ATWD and FADC means for each DOM in the data file.
    '''
    try:
        if not file.is_file():
            raise ValueError(f'The file {file} does not exist. Please check the file path and try again.')
        if file.suffix != '.npz':
            raise ValueError(f'The file {file} is not a .npz file. Please provide a .npz file and try again.')
        
        try:
            with np.load(file) as data:
                if 'atwd_mean' not in data or 'fadc_mean' not in data:
                    raise ValueError(f'The file {file} does not contain the required arrays "atwd_mean" and "fadc_mean". Please check the file and try again.')
...
def calc_llh(data_file: Path, 
    ...
    counts_test_data = create_mean_charge_hist(data_file, 
                                               nbins=nbins, lower_bound=lower_bound, upper_bound=upper_bound, density=False)
    ...
    for i in range(len(masked_corr)): #calculate the LLH in each bin and sum
        if masked_counts_test_data_corr[i] >=0:
            pois_corr = poisson.pmf(masked_counts_test_data_corr[i],
                                    masked_corr[i])
            logL_corr += np.log(pois_corr)

    for j in range(len(masked_uncorr)):
        if masked_counts_test_data_uncorr[j] >=0:
            pois_uncorr = poisson.pmf(masked_counts_test_data_uncorr[j],
                                      masked_uncorr[j])
    ...
    pearson_r = stats.pearsonr(
        flat_atwd_mean_data_test_masked,
        flat_fadc_mean_data_test_masked)[0]
...
parser.add_argument("-i","--inloc", type=Path, 
                    ...
                    required=True)
parser.add_argument("-o","--outloc", type=Path, 
...
parser.add_argument("--lower_bound", 
                    type=float, 
                    help='lower bound for the histogramming of the data file (should be the same as the templates). Default is 0.8.',default = 0.8)
parser.add_argument("--upper_bound", type=float, 
                    help='upper bound for the histogramming of the data file (should be the same as the templates). Default is 1.2.',default = 1.2)
parser.add_argument("--template_corrected", type=Path,
                    help='location of the corrected template .npz file.',
                    required=True)
parser.add_argument("--template_uncorrected", type=Path,
